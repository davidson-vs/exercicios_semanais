...
void buscar(FILA *f)
{
    ...
    if (posicao != NULL)
        printf("Elemento %i encontrado no endereco %p (busca sequencial).\n", ch, posicao);
    else
        printf("Nao foi possivel encontrar elemento %i.\n", ch);
    posicao = buscaSeqSent1(f, ch);
    if (posicao != NULL)
        printf("Elemento %i encontrado no endereco %p (busca sequencial sentinela 1).\n", ch, posicao);
    else
        printf("Nao foi possivel encontrar elemento %i.\n", ch);
    posicao = buscaSeqSent2(f, ch);
    if (posicao != NULL)
        printf("Elemento %i encontrado no endereco %p (busca sequencial sentinela 2).\n", ch, posicao);
    else
        printf("Nao foi possivel encontrar elemento %i.\n", ch);
}
...
void exibirPrimeiro(FILA *f)
{
    ...
    if (posicao != NULL)
        ...
        printf("Nao foi possivel encontrar o primeiro elemento (fila vazia).\n");
}
...
void exibirUltimo(FILA *f)
{
    ...
    if (posicao != NULL)
        ...
        printf("Nao foi possivel encontrar o ultimo elemento (fila vazia).\n");
}
...
void excluir(FILA *f)
{
    TIPOCHAVE ch;
    REGISTRO excluido;
    if (excluirDaFila(f, &excluido))
        printf("Elemento %i excluido corretamente.\n", excluido.chave);
    else
        printf("Nao foi possivel excluir elemento (fila vazia).\n");
}
